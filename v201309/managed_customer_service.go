...
var mccLayout = `{{define "T"}}<?xml version="1.0" encoding="UTF-8"?>
	...
	<env:Header>
		<wsdl:RequestHeader xmlns="https://adwords.google.com/api/adwords/cm/` + version + `">
	...
	<env:Body>
		<get xmlns="https://adwords.google.com/api/adwords/mcm/` + version + `">{{.Body}}
		</get>
	</env:Body>
</env:Envelope>{{end}}`
...
func (self *managedCustomerService) Get(v ManagedCustomerGetSelector) (*[]Entry, *[]Link, error) {
	...
	req, err := http.NewRequest("POST",
		"https://adwords"+self.SandboxUrl+".google.com/api/adwords/mcm/"+version+"/ManagedCustomerService",
		buffer)
	if err != nil {
		return nil, nil, err
	}
	...
	// fmt.Printf("\nsasGet%+v\n", sasGet)
	if sasGet.Body.Fault.FaultString != "" {
		return nil, nil, errors.New(sasGet.Body.Fault.FaultString)
	}

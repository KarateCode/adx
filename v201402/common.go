...
type Auth struct {
	DeveloperToken, ClientCustomerId, ClientId, Email, Password, Version, Type, ClientSecret, RefreshToken string
...
func New(auth Auth) *Adwords {
	...
	// conn := Conn{Auth: auth}
	conn := Conn{Auth: auth, Token: Authenticate(&auth)}
	if auth.Sandbox {
	...
	adwords := Adwords{
		...
		AdwordsUserListService:   adwordsUserListService(conn),
...
<env:Envelope 
	...
	xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:wsdl="https://adwords.google.com/api/adwords/{{.Mcc}}/{{.Auth.Version}}" xmlns:env="http://schemas.xmlsoap.org/soap/envelope/">
	<env:Header>
		<wsdl:RequestHeader xmlns="https://adwords.google.com/api/adwords/{{.Mcc}}/{{.Auth.Version}}">
			<userAgent>golang|adx</userAgent>
			<developerToken>{{.Auth.DeveloperToken}}</developerToken>
			<clientCustomerId>{{.Auth.ClientCustomerId}}</clientCustomerId>
		</wsdl:RequestHeader>
	</env:Header>
	<env:Body>
...
func Authenticate(auth *Auth) string {
	...
	res, err := client.PostForm("https://accounts.google.com/o/oauth2/token", params)
	...
	err = dec.Decode(auth) // already is a pointer
	if err != nil {
		panic(err)
	}
...
func CallApi(v interface{}, conn *Conn, service string, operation string, mcc string) (io.ReadCloser, error) {
	p, err := xml.Marshal(v)
	...
	req, err := http.NewRequest("POST", "https://adwords"+conn.SandboxUrl+".google.com/api/adwords/"+mcc+"/"+conn.Version+"/"+service, buffer)
	if err != nil {
		return nil, err
	}
	...
	req.Header.Add("clientCustomerId", conn.Auth.ClientCustomerId)
